...
const hasRepoLink = links.some((link) => {
  const url = new URL(link, "https://mcp-toolkit.moretes.com");
  return url.protocol === "https:" && url.hostname === "github.com" && url.pathname === "/fernandofatech/solution-architecture-mcp-toolkit";
});
...
const required = [
  ...
  [html.includes('rel="canonical" href="https://mcp-toolkit.moretes.com/"'), "index.html must include canonical domain"],
  ...
  [robots.includes("Sitemap: https://mcp-toolkit.moretes.com/sitemap.xml"), "robots.txt must point to sitemap"],
  [sitemap.includes("https://mcp-toolkit.moretes.com/"), "sitemap.xml must include canonical domain"],
